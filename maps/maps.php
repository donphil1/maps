<?php
$loc = dirname(__FILE__).'/mysqli/coord.db';
$db = new mysqli($loc,'SQL_OPEN_READWRITE' | 'SQL_OPEN_CREATE');
...
if (!empty($data) && is_object($data) && property_exists($data,'lat') && property_exists($data,'lon')){
    ...
    $src = 'SELECT name FROM mysqli WHERE type=\'table\' AND name=\'coordinates\'';
...
if (preg_match($regex,$data->lat) && preg_match($regex,$data->lon) )
	...
	$ins = "INSERT INTO coordinates (latitude,longitude) VALUES ('$lon','$lat')";
	$db->exec($ins);
	die();
}
?>
